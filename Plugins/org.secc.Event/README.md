...
| Generate Event Pass (`RequestEventPass`) | Public form that launches a workflow to request an Event Pass. The Text Message delivery option shows the carrier-required SMS disclosures (frequency, rates, STOP/HELP, linked se.church Privacy Policy and Mobile Terms) beneath the delivery-method field. | Block Title, Event Pass Workflow Type, Pass Person Attribute, Contact Fields Editable |
