...
//#endregion
//#region src/my-org-custom-app.ts
var Z = {
    title: "My Fast Rock Block",
    subtitle: "This UI is running as a standalone web component.",
    submitActionName: "SaveData"
}, Q = { status: "idle" }, $ = class extends J {
    constructor(...e) {
        super(...e), this.rockConfig = JSON.stringify(Z), this.serverState = JSON.stringify(Q), this.note = "";
    }
    get parsedConfig() {
        return this.parseJson(this.rockConfig, Z);
    }
    get parsedServerState() {
        return this.parseJson(this.serverState, Q);
    }
    parseJson(e, t) {
        try {
            return {
    ...
    onSubmit(e) {
        ...
        let t = {
            ...
            actionName: this.parsedConfig.submitActionName ?? "SaveData",
    ...
    createRequestId() {
        return typeof crypto < "u" && "randomUUID" in crypto ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
    }
    render() {
        let e = this.parsedConfig, t = this.parsedServerState, n = this.note.trim().length > 0 && t.status !== "saving";
        return L`
      <section>
        <h2>${e.title}</h2>
        <p>${e.subtitle ?? ""}</p>
        ...
        <form @submit=${this.onSubmit}>
          <label for="note">Note to send to Rock block action</label>
          <input
            id="note"
            name="note"
            type="text"
            .value=${this.note}
            @input=${this.onNoteInput}
            placeholder="Type a note"
          />

          <button type="submit" ?disabled=${!n}>
            ${t.status === "saving" ? "Saving..." : "Send to backend"}
          </button>
        </form>

        ${t.message ? L`<p class="message">${t.message}</p>` : null}
        ${t.savedNote ? L`<p class="saved">Last saved note: ${t.savedNote}</p>` : null}
      </section>
    `;
    }
    static {
...
X([Y({
    type: String,
    attribute: "rock-config"
})], $.prototype, "rockConfig", void 0), X([Y({
    type: String,
    attribute: "server-state"
})], $.prototype, "serverState", void 0), X([be()], $.prototype, "note", void 0), $ = X([_e("my-org-custom-app")], $);
//#endregion
export { $ as MyOrgCustomApp };
