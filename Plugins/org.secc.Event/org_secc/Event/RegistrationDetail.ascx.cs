...
namespace RockWeb.Plugins.org_secc.Event
{
    ...
    public partial class RegistrationDetail : RockBlock, IDetailBlock
    {
        ...
        protected void btnWorkflow_Click( object sender, EventArgs e )
        {
            ...
            HiddenField hiddenWorkflow = ( HiddenField ) item.FindControl( "hfWorkflow" );
            var workflowGuid = hiddenWorkflow.Value.AsGuidOrNull();
        ...
        private void ShowReadonlyDetails( Registration registration )
        {
            ...
            var workflowTypeGuids = GetAttributeValues( "Workflows" ).Select( w => w.AsGuid() ).ToList();
            if ( workflowTypeGuids.Any() )
            {
                rcwWorkflows.Visible = true;
                WorkflowTypeService workflowTypeService = new WorkflowTypeService( rockContext );
                var workflowTypes = workflowTypeService.GetByGuids( workflowTypeGuids ).ToList();
                rWorkflow.DataSource = workflowTypes;
                rWorkflow.DataBind();
            }
            registration.LoadAttributes();
