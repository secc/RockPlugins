    ...
    public partial class VerbDefinedType : Migration
    {
        public override void Up()
        {
            RockMigrationHelper.AddDefinedType( "Learning Management",
                ...
                Constants.DEFINEDTYPE_VERBS );
            RockMigrationHelper.AddDefinedTypeAttribute( Constants.DEFINEDTYPE_VERBS, Rock.SystemGuid.FieldType.TEXT,
                "Name", Constants.DEFINED_VALUE_ATTRIBUTE_KEY_NAME, "The human readable name for the verb.", 0, true, "",
                 false, true, Constants.DEFINED_VALUE_ATTRIBUTE_GUID_NAME );
            ...
            foreach ( var obj in dyn )
            {
                try
                {
                    ...
                    RockMigrationHelper.AddDefinedValue( Constants.DEFINEDTYPE_VERBS, uri, description, guid.ToString() );
                    RockMigrationHelper.AddDefinedValueAttributeValue( guid.ToString(), Constants.DEFINED_VALUE_ATTRIBUTE_GUID_NAME, name );
                }
                catch ( Exception e )
                {
