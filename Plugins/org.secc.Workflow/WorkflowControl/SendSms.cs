...
namespace org.secc.Workflow.WorkflowControl
{
    ...
    [WorkflowTextOrAttribute( "From", "Attribute Value", "The phone number or an attribute that contains the person or phone number that ther text should be sent from (will default to organization number 733733). <span class='tip tip-lava'></span>", false, "", "", 0, "From",
        new string[] { "Rock.Field.Types.TextFieldType", "Rock.Field.Types.PhoneNumberFieldType", "Rock.Field.Types.PersonFieldType" } )]
    ...
    public class SendSms : ActionComponent
    {
        ...
        public override bool Execute( RockContext rockContext, WorkflowAction action, Object entity, out List<string> errorMessages )
        {
            ...
            int? fromId = null;
            Guid? fromGuid = GetAttributeValue( action, "From",true ).AsGuidOrNull();
        
            if ( fromGuid.HasValue )
            {
                var fromAttribute = DefinedValueCache.Get( fromGuid.Value, rockContext );
                if ( fromAttribute != null )
                {
                    fromId = fromAttribute.Id;
                }
            }

            else
            {
                fromValue = fromValue.ResolveMergeFields( mergeFields );
            } 
