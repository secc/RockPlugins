...
";
        ...
        private IEnumerable<Rock.Chart.IChartData> GetAttendanceChartData()
        {
            ...
            var selectedCampusIds= CampusCache.All().Select( c => c.Id );

            string campusIds = string.Join( ",", selectedCampusIds );
            ...
            if ( !showNonAttenders )
            {
                ...
                qryTasks.Add( Task.Run( () =>
                {
                    DataTable dtAttendeeDates = AttendanceService.GetAttendanceAnalyticsAttendeeDates(
                        groupIdList, start, end, null, null, scheduleIdList ).Tables[0];
                ...
                qryTasks.Add( Task.Run( () =>
                {
                    dtAttendees = AttendanceService.GetAttendanceAnalyticsAttendees(
                        groupIdList, start, end, null, null, scheduleIdList, includeParents, includeChildren ).Tables[0];
                } ) );
                    ...
                    attendedMissedDateRange.End.HasValue )
                {
                    qryTasks.Add( Task.Run( () =>
                    {
                        ...
                        DataTable dtAttendeeDatesMissed = AttendanceService.GetAttendanceAnalyticsAttendeeDates(
                            ...
                            null, null, scheduleIdList ).Tables[0];
                ...
                qryTasks.Add( Task.Run( () =>
                {
                    dtAttendeeFirstDates = AttendanceService.GetAttendanceAnalyticsAttendeeFirstDates(
                        groupTypeIdList, groupIdList, start, end, null, null, scheduleIdList ).Tables[0];
                } ) );
            }
            else
            {
                qryTasks.Add( Task.Run( () =>
                {
                    DataSet ds = AttendanceService.GetAttendanceAnalyticsNonAttendees(
                        groupTypeIdList, groupIdList, start, end, null, null, scheduleIdList, includeParents, includeChildren );
        ...
        private void AddGroupTypeControls( GroupType groupType, HtmlGenericContainer liGroupTypeItem )
        {
            if ( !_addedGroupTypeIds.Contains( groupType.Id ) )
            {
                ...
                if ( groupType.Groups.Any() )
                {
                    ...
                    IEnumerable<Group> groups = groupType.Groups.Where( g => !g.ParentGroupId.HasValue || ( g.ParentGroup.GroupTypeId != groupType.Id ) );
                    if ( !cbShowInactive.Checked )
                    { groups = groups.Where( g => g.IsActive ); }
                    groups = groups.OrderBy( a => a.Order ).ThenBy( a => a.Name );
