...
namespace RockWeb.Blocks.CheckIn
{
    ...
    /// </summary>
    [DisplayName( "Group Attendance Analytics" )]
    ...
    public partial class GroupAttendanceAnalytics : RockBlock
    {
        #region Fields
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            cbShowInactive.Checked = GetGlobalPersonPreferences().GetValue( BlockCache.Guid.ToString() + "_showInactive" ).AsBoolean();
...
";
        ...
        private void SaveSettings()
        {
            ...
            preferences.SetValue( keyPrefix + "TemplateGroupTypeId", ddlAttendanceType.SelectedGroupTypeId.ToString() );

            preferences.SetValue( keyPrefix + "SlidingDateRange", drpSlidingDateRange.DelimitedValues );
            preferences.SetValue( keyPrefix + "GroupBy", hfGroupBy.Value );
            preferences.SetValue( keyPrefix + "GraphBy", hfGraphBy.Value );
            preferences.SetValue( keyPrefix + "CampusIds", clbCampuses.SelectedValues.AsDelimited( "," ) );
            preferences.SetValue( keyPrefix + "ScheduleIds", spSchedules.SelectedValues.ToList().AsDelimited( "," ) );
            preferences.SetValue( keyPrefix + "DataView", dvpDataView.SelectedValue );

            var selectedGroupIds = GetSelectedGroupIds( false );
            preferences.SetValue( keyPrefix + "GroupIds", selectedGroupIds.AsDelimited( "," ) );

            preferences.SetValue( keyPrefix + "ShowBy", hfShowBy.Value );

            preferences.SetValue( keyPrefix + "ViewBy", hfViewBy.Value );
            ...
            preferences.SetValue( keyPrefix + "AttendeesFilterByType", attendeesFilterBy.ConvertToInt().ToString() );
            preferences.SetValue( keyPrefix + "AttendeesFilterByVisit", ddlNthVisit.SelectedValue );
            preferences.SetValue( keyPrefix + "AttendeesFilterByPattern", string.Format( "{0}|{1}|{2}|{3}", tbPatternXTimes.Text, cbPatternAndMissed.Checked, tbPatternMissedXTimes.Text, drpPatternDateRange.DelimitedValues ) );

            preferences.Save();

            // Create URL for selected settings
            var pageReference = CurrentPageReference;
            foreach ( var settingKey in preferences.GetKeys().Where( k => k.StartsWith( keyPrefix ) ) )
            {
                string key = settingKey.Substring( keyPrefix.Length );
                pageReference.Parameters.AddOrReplace( key, preferences.GetValue( settingKey ) );
            }
        ...
        private void LoadSettings()
        {
            ...
            if ( !_isGroupSpecific )
            {
                ...
                cbIncludeGroupsWithoutSchedule.Checked = GetBlockPersonPreferences().GetValue( "IncludeGroupsWithoutSchedule" ).AsBooleanOrNull() ?? true;
                BuildGroupTypesUI( false );
            }
            ...
            if ( GetAttributeValue( "ShowCampusFilter" ).AsBoolean() )
            {
                ...
                else
                {
                    ...
                    if ( globalPreferences.ContainsKey( campusKey ) )
                    {
                        campusIdList = globalPreferences.GetValue( campusKey ).Split( ',' ).ToList();
                        clbCampuses.SetValues( campusIdList );
                    }
                    else
                    {
        ...
        private string GetSetting( string prefix, string key )
        {
            ...
            return GetGlobalPersonPreferences().GetValue( prefix + key );
        }
        ...
        private void BindChartAttendanceGrid()
        {
            ...
            BindChartAttendanceGrid( chartData.ToList() );
        }
        ...
        private void BindChartAttendanceGrid( List<Rock.Chart.IChartData> chartData )
        {
        ...
        private IEnumerable<Rock.Chart.IChartData> GetAttendanceChartData()
        {
            ...
            var chartData = GetChartData(
                hfGroupBy.Value.ConvertToEnumOrNull<CustomChartGroupBy>() ?? CustomChartGroupBy.Day,
        ...
        private IEnumerable<IChartData> GetChartData( CustomChartGroupBy groupBy = CustomChartGroupBy.Day, AttendanceGraphBy graphBy = AttendanceGraphBy.Total, DateTime? startDate = null, DateTime? endDate = null, List<int> groupIds = null, string campusIds = null, int? dataViewId = null, string scheduleIds = null )
        {
            ...
            if ( campusIdList.Any() )
            {
                if ( includeNullCampus )
                {
                    ...
                    qryAttendance = qryAttendance.Where( o => ( o.Group.CampusId.HasValue && campusIdList.Contains( o.Group.CampusId.Value ) ) || !o.Group.CampusId.HasValue ).ToList();
                }
                else
                {
                    // only show records that have a campusId in the campusIdList
                    qryAttendance = qryAttendance.Where( o => o.Group.CampusId.HasValue && campusIdList.Contains( o.Group.CampusId.Value ) ).ToList();
                }
            }
            else if ( includeNullCampus )
            {
                ...
                qryAttendance = qryAttendance.Where( o => !o.Group.CampusId.HasValue ).ToList();
            }
            ...
            if ( scheduleIdList.Any() )
            {
                qryAttendance = qryAttendance.Where( o => o.ScheduleId.HasValue && scheduleIdList.Contains( o.ScheduleId.Value ) ).ToList();
            }

            var qryAttendanceWithSummaryDateTime = GetAttendanceWithSummaryDateTime( qryAttendance.SelectMany( o => o.Attendees.Where( a => a.DidAttend != null && a.DidAttend.Value ) ), groupBy ).ToList();
            ...
            if ( groupBy == CustomChartGroupBy.Week )
            {
                foreach ( var attendanceOccurance in qryAttendance )
                {
                    attendanceOccurance.OccurrenceDate = attendanceOccurance.OccurrenceDate.StartOfWeek( DayOfWeek.Sunday );
                }
            }
            else if ( groupBy == CustomChartGroupBy.Month )
            {
                foreach ( var attendanceOccurance in qryAttendance )
                {
                    attendanceOccurance.OccurrenceDate = new DateTime( attendanceOccurance.OccurrenceDate.Year, attendanceOccurance.OccurrenceDate.Month, 1 );
                }
            }
            else if ( groupBy == CustomChartGroupBy.Year )
            {
                foreach ( var attendanceOccurance in qryAttendance )
                {
                    attendanceOccurance.OccurrenceDate = new DateTime( attendanceOccurance.OccurrenceDate.Year, 1, 1 );
                }
            }
            ...
            summaryQry.AddRange(
                  ...
                  {
                      SummaryDateTime = ao.OccurrenceDate.Date,
                      Campus = new
                      {
            ...
            summaryQry = summaryQry.OrderBy( s => s.SummaryDateTime ).ToList();
        ...
        {
            IEnumerable<AttendanceService.AttendanceWithSummaryDateTime> qryAttendanceGroupedBy;

            if ( summarizeBy == CustomChartGroupBy.Week )
            {
                qryAttendanceGroupedBy = qryAttendance.Select( a => new AttendanceService.AttendanceWithSummaryDateTime
                {
                    SummaryDateTime = a.Occurrence.OccurrenceDate.StartOfWeek( DayOfWeek.Sunday ),
                    Attendance = a
                } );
            }
            else if ( summarizeBy == CustomChartGroupBy.Month )
            {
                qryAttendanceGroupedBy = qryAttendance.Select( a => new AttendanceService.AttendanceWithSummaryDateTime
                {
                    SummaryDateTime = new DateTime( a.Occurrence.OccurrenceDate.Year, a.Occurrence.OccurrenceDate.Month, 1 ),
                    Attendance = a
                } );
            }
            else if ( summarizeBy == CustomChartGroupBy.Year )
            {
                qryAttendanceGroupedBy = qryAttendance.Select( a => new AttendanceService.AttendanceWithSummaryDateTime
                {
                    SummaryDateTime = new DateTime( a.Occurrence.OccurrenceDate.Year, 1, 1 ),
                    Attendance = a
                } );
            }
            else
            {
                qryAttendanceGroupedBy = qryAttendance.Select( a => new AttendanceService.AttendanceWithSummaryDateTime
                {
                    SummaryDateTime = a.Occurrence.OccurrenceDate,
                    Attendance = a
                } );
            }
        ...
        private enum CustomChartGroupBy
        {
            /// <summary>
        ...
        private void AddGroupTypeControls( GroupType groupType, HtmlGenericContainer liGroupTypeItem )
        {
            if ( !_addedGroupTypeIds.Contains( groupType.Id ) )
            {
                ...
                bool showInactive = GetGlobalPersonPreferences().GetValue( BlockCache.Guid.ToString() + "_showInactive" ).AsBoolean();
        ...
        {
            var preferences = GetGlobalPersonPreferences();
            preferences.SetValue( BlockCache.Guid.ToString() + "_showInactive", cbShowInactive.Checked.ToString() );
            preferences.Save();
            BuildGroupTypesUI( true );
        }
