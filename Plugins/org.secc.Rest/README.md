...
| Confirmation email | Sent to the person's **stored** address. `RockMessage.Send()` returns `false` (doesn't throw) on failure; on the username path a failed send deletes the just-created login and returns `503` so retry isn't blocked by the username-uniqueness check. The email-only/SMS path is **not** cleaned up on a failed send (no user-chosen username to trap) — note its `200` response still reports "email sent". |
